...
tpu_assunto_read <- function(busca = NULL, ini = Sys.Date(), fim = Sys.Date()) {
  ...
  if (length(files) == 0) {
    rlang::abort(c(
      "Nenhuma TPU disponível para o período indicado.",
      "Verifique as datas disponíveis em tpur::assuntos."
    ))
  }
...
tpu_assuntos_busca_vetorizada_codigo <- function(buscacodigo = NULL, ini = Sys.Date(), fim = Sys.Date()) {
